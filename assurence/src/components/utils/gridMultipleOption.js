...
<div  className="flex flex-col mb-2   text-center  justify-center w-4/4 xs:w-36 md:w-44 items-center focus:outline-none relative mx-auto" style={{ pointerEvents:"none"}}
  id= {this.props.id}>
     ...
     {(this.props.choix).includes(this.props.id) ? <img   src={this.props.src_c} alt="logo" className=" h-20"  style={{ pointerEvents:"none"}}/>: 
     <img   src={this.props.src} alt="logo" className=" h-20"  style={{ pointerEvents:"none"}}/>}
      ...
      style={{ pointerEvents:"none"}}><p className="text-sm font-bold text-[#444] text-4 font-sans" style={{ pointerEvents:"none"}}>{this.props.text_option}</p></div>
  {this.props.src ?   <div
    ...
    <DoneIcon   style={{ fill: '#e0dfda' }}  />  </div>:  <div
